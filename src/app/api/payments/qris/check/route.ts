...
async function handleCheckStatus(request: Request) {
  try {
    ...
    if (xoftwareData.transaction_id || xoftwareData.ref_id) {
      const refToSearch = xoftwareData.ref_id || xoftwareData.transaction_id;
      ...
      if (payment && xoftwareData.status) {
        ...
        const newStatus = xoftwareData.status === 'success' ? 'paid' :
                          xoftwareData.status === 'fail' ? 'failed' : payment.status;
    ...
    return successResponse({
      ...
      reff_id: xoftwareData.ref_id || txId,
